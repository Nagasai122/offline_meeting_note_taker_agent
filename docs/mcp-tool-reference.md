...
| `extract_action_items` | `TRANSCRIBED → EXTRACTED` (or `→ FAILED`) | The one LLM-backed step. Writes `data/meetings/<session_id>.actions.json`, `.summary.md`, and `.mom.md` (type-aware Minutes of Meeting — see `docs/architecture_v2.md` §5). Raises `ExtractionError` if the model's response is not a valid JSON **object** with `"summary"` (string) and `"action_items"` (array of `{description, owner, due_date, priority, ...}` objects) keys — a markdown code fence around the JSON is tolerated and stripped, nothing else is. Meeting type and recording date are auto-resolved from the session's `.type` file/slug and transcript mtime; not exposed as separate MCP tool arguments. |
